...
/* USER CODE BEGIN PTD */
#define JOIN_LED_TIME 500
#define NORMAL_LED_TIME 1000
#define RX_LED_TIME 600
#define TX_LED_TIME 300
...
// buffer for host uart rx
uint8_t uart_rx_buffer[UART_RX_BUFFER_SIZE];
// a label to indicate receive data from host
// buffer for lora to send
uint8_t lora_send_buffer[UART_RX_BUFFER_SIZE];
...
static UTIL_TIMER_Object_t UartTimer;

//crc algorithm
uint8_t CRC_Check(uint8_t *data, size_t len)
{
    uint8_t crc = 0xff;
    size_t i, j;
    for (i = 0; i < len; i++) {
...
static void OnUartTimer(void*){
	//clear existing buffer
	HAL_UART_Receive(&hlpuart1, uart_rx_buffer , 100, 10);

	HAL_UART_Transmit(&hlpuart1, (uint8_t*)"GET\r\n", 5, 10);
	if(HAL_UART_Receive(&hlpuart1, uart_rx_buffer , 8, 100)!=HAL_OK){
		APP_LOG(TS_OFF, VLEVEL_L,"Reading Data Fails\r\n");
		lora_send_buffer[0]=0x01;
		return;
	}
	if(memcmp(uart_rx_buffer,"coed",4)!=0){
		APP_LOG(TS_OFF, VLEVEL_L,"Reading Data Fails\r\n");
		lora_send_buffer[0]=0x01;
		return;
	}

	memcpy(lora_send_buffer,uart_rx_buffer+4,4);

	if(lora_send_buffer[1] & 0x01){
		APP_LOG(TS_OFF, VLEVEL_L,"Reading Data From ESP32, But No eGuage Data");
		lora_send_buffer[0]=0x00;
		return;
	}

	uint16_t count = lora_send_buffer[2]<<8 | lora_send_buffer[3];
	if(count>24){
		APP_LOG(TS_OFF, VLEVEL_L,"Reading Data Error\r\n");
		lora_send_buffer[0]=0x01;
		return;
	}

	if(HAL_UART_Receive(&hlpuart1, uart_rx_buffer , 4*count+4,500)!=HAL_OK){
		APP_LOG(TS_OFF, VLEVEL_L,"Reading Data Fails\r\n");
		lora_send_buffer[0]=0x01;
		return;
	}

	if(memcmp(uart_rx_buffer+4*count+1,"su\n",3)!=0){
		APP_LOG(TS_OFF, VLEVEL_L,"Reading Data ERROR\r\n");
		lora_send_buffer[0]=0x01;
		return;
	}

	if(uart_rx_buffer[4*count]!=CRC_Check(uart_rx_buffer, 4*count)){
		APP_LOG(TS_OFF, VLEVEL_L,"CRC Check ERROR\r\n");
		return;
	}
	lora_send_buffer[0]=0;
	memcpy(lora_send_buffer+4,uart_rx_buffer,4*count);

	APP_LOG(TS_OFF, VLEVEL_L,"Receive Data\r\n");
...
void LoRaWAN_Init(void)
{
  ...
  /* USER CODE BEGIN LoRaWAN_Init_1 */
	...
	UTIL_TIMER_Create(&UartTimer, 15000, UTIL_TIMER_PERIODIC,
				OnUartTimer, NULL);


	  UTIL_SEQ_RegTask((1 << CFG_SEQ_Task_LoraTxFlash), UTIL_SEQ_RFU, LoraTxFlash);
  ...
  /* USER CODE BEGIN LoRaWAN_Init_Last */
	...
	//HAL_UARTEx_ReceiveToIdle_IT(&hlpuart1, uart_rx_buffer, UART_RX_BUFFER_SIZE);
...
static void SendTxData(void)
{
  /* USER CODE BEGIN SendTxData_1 */
	...
	uint16_t count = (uint16_t) lora_send_buffer[2] << 8 | lora_send_buffer[3];
	int i = 4;
	for (int j = 0; j < i; j++) {
		AppData.Buffer[j] = lora_send_buffer[j];
	}
	if(lora_send_buffer[0] ==0 && lora_send_buffer[1] == 0){
		i = 4*count+4;
		for (int j = 4; j < i; j++) {
			AppData.Buffer[j] = lora_send_buffer[j];
		}
	...
	//memcpy(AppData.Buffer,lora_send_buffer,i*4+4);
	    ...
	    !LmHandlerIsBusy())
	{
		...
		if (LORAMAC_HANDLER_SUCCESS == status) {
			//UTIL_TIMER_Start(&LedFlashTimer);
			APP_LOG(TS_ON, VLEVEL_L, "SEND REQUEST\r\n");
			UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_LoraTxFlash),
								CFG_SEQ_Prio_0);
	...
	{
	    APP_LOG(TS_OFF, VLEVEL_L, "Join not done or MAC busy\n");
	}
	...
	if (params != NULL) {
		...
		case LORAMAC_HANDLER_BEACON_RX: {
			APP_LOG(TS_OFF, VLEVEL_M,
					...
					params->Info.Datarate, params->Info.Rssi, params->Info.Snr,
					params->Info.Frequency, params->Info.Time.Seconds,
					params->Info.GwSpecific.InfoDesc,
					params->Info.GwSpecific.Info[0],
